...
export function LoansSection({ loans, setLoans, payments, setPayments, activeTab, personName }: LoansSectionProps) {
  const [showArchived, setShowArchived] = useState(false);

  const allUserLoans = loans.filter(l => l.owner === activeTab);
  ...
  // Función para obtener pagos de un préstamo específico (de todos los meses)
  const getPaymentsForLoan = (loanId: string) => {
    return payments.filter(p => p.type === 'loan' && p.referenceId === loanId);
  };

  // Función para calcular el total pagado de todos los tiempos
  const getTotalPaidForLoan = (loanId: string) => {
    // Aquí solo tenemos pagos del mes actual, en una implementación completa
    // necesitarías obtener pagos de todos los meses
      ...
      <div className="space-y-3 sm:space-y-4">
        {visibleLoans.length === 0 && !showArchived && (
          <div className="text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
            ...
            <p className="text-gray-500 font-medium mb-1">No tienes préstamos registrados</p>
            <p className="text-gray-400 text-sm">Registra el dinero que has prestado para hacer seguimiento de pagos</p>
          </div>
        )}

        {visibleLoans.length === 0 && showArchived && (
          <div className="text-center py-12 bg-amber-50 rounded-lg border-2 border-dashed border-amber-300">
            <Archive className="w-12 h-12 text-amber-400 mx-auto mb-3" />
            <p className="text-amber-600 font-medium mb-1">No hay préstamos archivados</p>
            <p className="text-amber-500 text-sm">Los préstamos archivados aparecerán aquí</p>
          </div>
        )}
          ...
          return (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Monto Total Prestado</label>
                  <OptimizedInput
                    type="number"
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-700 font-medium">Progreso de Recuperación</span>
              ...
              <div className="bg-white rounded-lg p-3 space-y-2">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="text-sm font-semibold text-gray-700">Pagos Esperados (Este Mes)</h4>
                  <button
                    onClick={() => {
                ...
                {loanPayments.length === 0 && (
                  <p className="text-sm text-gray-400 text-center py-2">
                    No hay pagos configurados para este mes. Haz clic en "Agregar Pago" para crear la lista de pagos esperados.
                  </p>
                )}
                  ...
                  return (
                    ...
                    >
                      ...
                      <input
                        ...
                        className={`px-2 py-1 border border-gray-300 rounded text-xs focus:ring-2 focus:ring-amber-500 ${
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <button
                  onClick={() => {
                    if (confirm('¿Eliminar este préstamo permanentemente? Esta acción no se puede deshacer.')) {
                      // Eliminar también todos los pagos asociados
                      setPayments(payments.filter(p => !(p.type === 'loan' && p.referenceId === loan.id)));
        ...
        <button
          ...
          className="w-full py-2 px-4 bg-amber-100 text-amber-700 rounded-lg hover:bg-amber-200 transition-colors"
        >
          + Agregar Préstamo
        </button>
      </div>
